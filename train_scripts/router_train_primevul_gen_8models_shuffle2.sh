...
for seed in "${seeds[@]}"; do
    PYTHONPATH=. ./venv/bin/python3 -u train_files/train_router_primevul_gen_8models_shuffle2.py --training_steps ${training_steps} --top_k ${top_k} --last_k ${last_k} --learning_rate ${learning_rate} --eval_steps 50 --tempreture ${tempreture} --similarity_function ${similarity_function} --sample_loss_weight ${sample_loss_weight} --cluster_loss_weight ${cluster_loss_weight} --seed ${seed} --batch_size 64 --training_samples_per_dataset 8405 \
